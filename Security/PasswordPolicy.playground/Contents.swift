...
class LocalPassPolicy {
    ...
    var passPolicies = [AnyHashable: Any]() // holds the raw policy from the node
    ...
    func validateUsingPredicate(password: String) {
        do {
            let node = try ODNode.init(session: session, type: ODNodeType(kODNodeTypeLocalNodes))
            let passPolicies = try node.accountPolicies()
        } catch {
...
//    testPolicy.validate(pass: "123")
    testPolicy.validateUsingPredicate(password: "123")
