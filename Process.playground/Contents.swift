...
//}
//
//func debugGetSystemSettings() -> String {
//     return cliTask("system_profiler -detailLevel baisc SPSoftwareDataType SPHardwareDataType SPiBridgeDataType SPDiagnosticsDataType SPStorageDataType SPFirewallDataType SPEthernetDataType SPAirPortDataType", waitForTermination: true)
//}
//
////let output = debugGetSystemSettings()

let task = Process()
task.executableURL = URL(fileURLWithPath: "/bin/sh")
task.arguments = ["-c", "/usr/bin/ldapsearch", "-w", "\"${DN_CRED}\""]
task.environment = ["DN_CRED": "secret"]
//let inData = Data("foo".utf8)
let inPipe = Pipe()
let outPipe = Pipe()
let errPipe = Pipe()
...
try inPipe.fileHandleForWriting.write(contentsOf: inData)
let input = try inPipe.fileHandleForReading.readToEnd()


//let dat = inPipe.fileHandleForReading.availableData
task.launch()
...
var outputString = ""
while true {
    let data = outPipe.fileHandleForReading.readData(ofLength: 1024)

    if data.count <= 0 {
        break
    }

    outputString += String(decoding: data, as: UTF8.self)
}

let error = errPipe.fileHandleForReading.readDataToEndOfFile()
let errString = String(decoding: error, as: UTF8.self)

 outputString + " " + errString
